...
function createCustomer(id: number): ICASOCNT {
  const name =
...
function createDisputeType(id: number): ICASODPT {
  return {
...
function createDisputeHandler(id: number): ICASODPH {
  return {
...
// FULL MOCK
export function generateMockCommonData(): ICommonDataResponse {
  const customers: ICASOCNT[] = Array.from({ length: 15 }).map((_, i) =>
    createCustomer(i + 1)
  );

  const disputeTypes: ICASODPT[] = Array.from({ length: 5 }).map((_, i) =>
