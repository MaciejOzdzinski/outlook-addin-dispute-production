...
function generateRandomDisputePositions(count) {
    return Array.from({ length: count }).map((_, i) => ({
        ...
        DPMSGD: randomText(),
        DPMSGA: randomText(),
        DPADAT: Number(new Date(2024, rand(0, 11), rand(1, 28))
            .toISOString()
            .slice(0, 10)
            .replace(/-/g, "")),
    }));
...
export function generateMockInvoicesForCustomer(customerNumber, count = 15) {
