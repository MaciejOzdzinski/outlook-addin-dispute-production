...
export async function getMimeObjectFromOutlookItem(item) {
    // 1. HTML
    const originalHtml = await new Promise((resolve, reject) => {
    ...
    const cidMatches = Array.from(originalHtml.matchAll(/<img[^>]+src=(["'])cid:([^"'>]+)\1/gi)).map((m) => m[2]);
    // 4. Załączniki
    ...
    for (const att of attList) {
        ...
        if (!val)
            continue;
        const contentTypeFromAtt = att.contentType || inferMimeTypeFromName(att.name);
        if (att.isInline) {
    ...
    for (const pic of inlinePictures) {
        const cidPattern = new RegExp(`src=(["'])cid:${escapeRegExp(pic.contentId)}\\1`, "gi");
