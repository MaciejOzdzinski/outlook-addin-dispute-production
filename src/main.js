...
function detectInitialTheme() {
    ...
    if (win.Office && win.Office.context && win.Office.context.officeTheme) {
        ...
        const bg = officeTheme.bodyBackgroundColor;
    ...
    React.useEffect(() => {
        ...
        Office.onReady(() => {
            try {
                ...
                console.log("Używany motyw Fluent:", theme === webDarkTheme ? "dark" : "light");
                const bg = officeTheme.bodyBackgroundColor;
                const lum = getLuminanceFromHex(bg || "#ffffff");
                ...
                if (eventType && Office.context.document?.addHandlerAsync) {
                    Office.context.document.addHandlerAsync(eventType, () => {
                        const t = Office.context.officeTheme;
                        const bg2 = t?.bodyBackgroundColor;
                ...
                console.warn("Cannot read Office theme, falling back to current theme.", e);
            }
        });
    }, []);
    return (_jsx(FluentProvider, { theme: theme, children: _jsx(App, {}) }));
}
function render() {
